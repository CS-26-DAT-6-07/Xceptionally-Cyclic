...
class TreeStrategy(FedAvg):
   ...
   #A bunch of stuff had to change to fit flower 1.29
    ...
    def aggregate_train(
        ...
        client_ids = []
        feature_vectors = []
        valid_replies = [] #This is just to keep track of replies that did not crash

        for reply in replies_list:

            #Check if the message has content (if it crashed it wont have content)
        ...
        #STEP 3: CREATE NEW EDGE GROUPS (Right now hard-coded to two edge groups)
        new_edge_groups = {0: [], 1: []}
        for client_id, cluster_id in zip(client_ids, cluster_labels):
            new_edge_groups[int(cluster_id)].append(int(client_id))
